...
PUBLIC void keyboard_read(TTY* p_tty)
{
	...
	// 如果缓冲区有字符则表示有扫描码，需要进行读取，该缓冲区开始时关闭中断，离开缓冲区开中断
	if(kb_in.count > 0){
		code_with_E0 = 0;

		scan_code = get_byte_from_kbuf();

		// 除去 0xE1 和 0XE0 外其他扫描码都是单字符
		/* 下面开始解析扫描码 */
		if (scan_code == 0xE1) {
			int i;
